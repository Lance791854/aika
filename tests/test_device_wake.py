...
def test_full_command_forwards_but_opens_no_window() -> None:
    # "AIKA sausage ten minutes" then "I'm talking to my robot" — the second
    # must be dropped. A complete command doesn't leave the mic open.
    forward, armed = agent_aika.device_gate("aika steak four minutes", 0.0, now=1000.0)
    assert forward
    assert armed == 0.0


def test_bare_wake_opens_window() -> None:
    forward, armed = agent_aika.device_gate("aika.", 0.0, now=1000.0)
    assert forward
...
def test_armed_window_forwards_once_then_closes() -> None:
    forward, armed = agent_aika.device_gate(
        "four minutes for the lamb", 1005.0, now=1000.0
    )
    assert forward
    assert armed == 0.0  # window is used up by one follow-up
...
async def test_device_mode_drops_empty_turns() -> None:
    ...
    with pytest.raises(StopResponse):
        await a.on_user_turn_completed(None, llm.ChatMessage(role="user", content=[""]))
